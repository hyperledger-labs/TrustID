...
function createDeclaration(context, node, kind, name) {
    ...
    let isExported;
    if (kind === index_1.ReflectionKind.ExternalModule || kind === index_1.ReflectionKind.Global) {
        isExported = true;
        hasComment = true;
    }
    ...
    else if (container.kindOf([index_1.ReflectionKind.Module, index_1.ReflectionKind.ExternalModule])) {
    ...
    if ((!isExported && context.converter.excludeNotExported)
        ||
            (context.converter.excludeNotDocumented && kind !== index_1.ReflectionKind.EnumMember && !hasComment)) {
        return;
    }
    ...
    if (!child) {
        ...
        if (child) {
            children.push(child);
            context.registerReflection(child, (_b = context.getSymbolAtLocation(node), (_b !== null && _b !== void 0 ? _b : node.symbol)));
        }
    }
    else {
